...
class resnet_v1_101_flownet_deeplab(Symbol):
    ...
    def get_train_symbol(self, cfg):
        ...
        data_ref_prev = mx.sym.Variable(name="data_ref_prev")
        data_ref_next = mx.sym.Variable(name="data_ref_next")
        ...
        conv_r_feat = conv_split[1]

        m_vec_split = mx.sym.split(data, axis=0, num_outputs=2)
        m_vec_l = m_vec_split[0]
        m_vec_r = m_vec_split[1]

        m_vec_l_grid = mx.sym.GridGenerator(data=m_vec_l, transform_type='warp', name='flow_grid')
        warp_conv_l_feat = mx.sym.BilinearSampler(data=conv_l_feat, grid=m_vec_l_grid, name='warping_feat')
        ...
        croped_score = mx.symbol.Crop(*[upsampling, data_ref_prev], offset=(8, 8), name='croped_score')
        ...
        group = mx.sym.Group([softmax, data, eq_flag])
        self.sym = group
        return group
    ...
    def get_key_test_symbol(self, cfg):
        ...
        feat_forw = mx.sym.Variable(name="feat_forw")
        ...
        group = mx.sym.Group([m_vec, feat_forw, feat_back, conv_feat, croped_score])
        self.sym = group
        return group
    ...
    def init_weight(self, cfg, arg_params, aux_params):
        arg_params['fuse_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['fuse_weight'])
        arg_params['fuse_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['fuse_bias'])
