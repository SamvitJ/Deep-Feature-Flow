...
def main():
    ...
    # load demo data
    if has_gt:
        image_names = sorted(glob.glob(cur_path + '/../demo/cityscapes_frankfurt_all_c' + str(interv) + '/*.png'))
    ...
    for idx, im_name in enumerate(image_names):
        ...
        data.append({
            ...
            'data_next_key': np.zeros(np.shape(key_im_tensor)),
            'feat_next_key': np.zeros((1,config.network.DFF_FEAT_DIM,1,1)),
        })


    # get predictor
    data_names = ['data', 'data_key', 'feat_key', 'data_next_key', 'feat_next_key']
    label_names = []
    data = [[mx.nd.array(data[i][name]) for name in data_names] for i in xrange(len(data))]
    max_data_shape = [[('data', (1, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                       ('data_key', (1, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                       ('data_next_key', (1, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES])))]]
    ...
    for j in xrange(2):
        data_batch = mx.io.DataBatch(data=[data[j]], label=[], pad=0, index=0,
                                     ...
                                     provide_label=[None])
        next_idx = j + key_frame_interval - 1 if j + key_frame_interval <= len(image_names) else j
        ...
        if j % key_frame_interval == 0:
            ...
            data_batch.data[0][-3] = feat
            data_batch.provide_data[0][-3] = ('feat_key', feat.shape)
            data_batch.data[0][-2] = data_next
            data_batch.provide_data[0][-2] = ('data_next_key', data_next.shape)
            data_batch.data[0][-1] = feat_next
            data_batch.provide_data[0][-1] = ('feat_next_key', feat_next.shape)
    ...
    for idx, im_name in enumerate(image_names):
        data_batch = mx.io.DataBatch(data=[data[idx]], label=[], pad=0, index=idx,
                                     ...
                                     provide_label=[None])
        next_idx = idx + key_frame_interval - 1 if idx + key_frame_interval <= len(image_names) else idx
        data_batch_next = mx.io.DataBatch(data=[data[next_idx]], label=[], pad=0, index=next_idx,
                                     provide_data=[[(k, v.shape) for k, v in zip(data_names, data[next_idx])]],
                                     provide_label=[None])
        scales = [data_batch.data[i][1].asnumpy()[0, 2] for i in xrange(len(data_batch.data))]
