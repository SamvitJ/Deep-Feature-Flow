...
def train_net(args, ctx, pretrained, pretrained_flow, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    # infer max shape
    max_data_shape = [('data', (config.TRAIN.BATCH_IMAGES * 2, 2, 64, 128)),
                      ('data_ref_prev', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                      ('data_ref_next', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
                      ('eq_flag', (1,))]
