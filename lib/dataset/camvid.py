...
class CamVid(IMDB):
    def __init__(self, image_set, root_path, dataset_path, result_path=None):
        ...
        super(CamVid, self).__init__('camvid', image_set, root_path, dataset_path, result_path)  # set self.name
    ...
    def load_image_set_index(self):
        ...
        for i, image_name in enumerate(image_name_set):
            ...
            if ext == 'png':
                index_set[valid_index_count] = "_".join(splited_name_set[:len(splited_name_set)])
                valid_index_count += 1
    ...
    def image_path_from_index(self, index):
        ...
        image_file = os.path.join(self.data_path, self.image_set_main_folder, self.image_set_sub_folder, index_folder, index)
    ...
    def annotation_path_from_index(self, index):
        ...
        """
        index_folder = index.split('_')[0]
        index_file = index.split('_')[1].split('.')[0]
        image_file = os.path.join(self.data_path, 'labels', self.image_set_sub_folder, index_folder, index_folder + '_' + index_file + '_L.png')
        assert os.path.exists(image_file), 'Path does not exist: {}'.format(image_file)
        return image_file
